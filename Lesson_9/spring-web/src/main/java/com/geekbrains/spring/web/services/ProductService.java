...
import com.geekbrains.spring.web.repositories.spetification.ProductSpecification;
...
public class ProductService {
    ...
    public Page<Product> findAllProduct(Integer p, Integer maxScore, Integer minScore, String partName){
        Specification<Product> specProduct = Specification.where(null);
        if(minScore != null){
            specProduct = specProduct.and(ProductSpecification.scoreGreaterOrElseThan(minScore));
        }
        if(maxScore != null){
            specProduct = specProduct.and(ProductSpecification.lessThanOrEqualTo(maxScore));
        }
        if(partName != null){
            specProduct = specProduct.and(ProductSpecification.nameLike(partName));
        }
        return productRepository.findAll(specProduct, PageRequest.of(p -1, 5));
    }
